---
created: 2026-05-06, 13:08
modified: 2026-05-31, 21:55
---
...
![[学习笔记_S03_计划优先 2026-05-31 21.40.07.excalidraw|996]]
...
sequenceDiagram
    ...
    Note over LLM: 初始状态：所有任务 pending [ ]

    LLM->>+Agent: tool_use("todo", items=[{status: "in_progress"}])
    Agent->>+TodoMgr: TODO.update(items)
    TodoMgr-->>-Agent: "[>] #1: 设计UI (in_progress)\n[ ] #2: 实现逻辑 (pending)"
    Agent-->>-LLM: tool_result

    Note over LLM: rounds_since_todo = 0 (刚用了todo)
    ...
    LLM->>+Agent: tool_use("todo", items=[{status: "completed"}, {status: "in_progress"}])
    Agent->>+TodoMgr: TODO.update(items)
    TodoMgr-->>-Agent: "[x] #1: 设计UI (completed)\n[>] #2: 实现逻辑 (in_progress)"
    Agent-->>-LLM: tool_result

    Note over LLM: rounds_since_todo = 0 (又用了todo)

    LLM->>+Agent: tool_use("write_file", ...)
    Agent->>+FileSys: 写入 calculator.py
    FileSys-->>-Agent: "Wrote 800 bytes"
    Agent-->>-LLM: tool_result

    Note over LLM: rounds_since_todo = 1 (还没超过3次)

    LLM->>+Agent: tool_use("todo", items=[{status: "completed"}])
    Agent->>+TodoMgr: TODO.update(items)
    TodoMgr-->>-Agent: "[x] #1: 设计UI (completed)\n[x] #2: 实现逻辑 (completed)"
    Agent-->>-LLM: tool_result
    ...
    Note over LLM: 全部完成！状态流转：pending → in_progress → completed
```
