...
```mermaid
sequenceDiagram
    participant Agent as 自主代理 (线程)
    participant BUS as MessageBus
    participant Tasks as 任务面板 (.tasks/)
    ...
    Agent-->>-Agent: stop_reason != tool_use 或 调用 idle
    ...
    loop 每 5 秒轮询 (最多 12 次)
        ...
        Tasks-->>-Agent: [task_5: {status: "pending", owner: null, blockedBy: []}]

        Note over Agent: 发现未认领任务<br/>claim_task(5, "alice")
        Agent->>+Tasks: 更新 task_5.json: owner="alice", status="in_progress"
        Tasks-->>-Agent: "Claimed task #5 for alice"

        Agent->>Agent: 注入 <auto-claimed> 消息到 messages
        Agent-->>-Agent: resume = True
    end

    Note over Agent: 恢复 WORK 阶段<br/>继续处理任务

    loop 继续工作...
    end

    Note over Agent: idle 60s 无事发生<br/>should_exit = True<br/>线程退出
```
...
```mermaid
sequenceDiagram
    participant Alice as 代理 Alice (线程)
    participant Bob as 代理 Bob (线程)
    participant Tasks as 任务面板 (.tasks/)
    participant Lock as _claim_lock

    Alice->>+Tasks: scan_unclaimed_tasks()
    Tasks-->>-Alice: [{id: 5, subject: "实现功能 X"}]

    Bob->>+Tasks: scan_unclaimed_tasks()
    Tasks-->>-Bob: [{id: 5, subject: "实现功能 X"}]

    Alice->>+Lock: 获取锁
    Alice->>+Tasks: claim_task(5, "alice")
    Tasks-->>-Alice: "Claimed task #5 for alice"
    Lock-->>-Alice: 释放锁

    Bob->>+Lock: 获取锁
    Bob->>+Tasks: claim_task(5, "bob")
    Tasks-->>-Bob: "Error: Task 5 has already been claimed by alice"
    Lock-->>-Bob: 释放锁

    Note over Alice,Bob: Bob 会继续扫描其他未认领任务
```
